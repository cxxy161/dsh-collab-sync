 ...
 *  4. 多终端感知：GUI beacon + /collab/panel 协作面板 + collab_status/repair 工具。
 *
...
import { beaconScript, beaconTag } from './beacon.js'
import { createBindHandler, effectiveBindStatus } from './bind.js'
...
export function apply(ctx, config = {}) {
  ...
  if (!acquired.ok) {
    if (acquired.reason === 'readonly') {
      logger.info?.('dsh-collab-sync: readonly follower mode — not acquiring writer lock')
    } else if (acquired.reason === 'conflict') {
      const holder = acquired.holder ?? {}
      if (cfg.mode === 'writer') {
        // Strict single-backend mode: a second writer fails fast.
        throw new Error(
          `dsh-collab-sync: another dsh instance already owns session root ${acquired.path}\n` +
            `  holder: pid ${holder.pid ?? '?'} @ ${holder.hostname ?? '?'}, started ${holder.startedAt ?? '?'}` +
            (holder.port ? `, port ${holder.port}` : '') +
            `\nSingle-backend contract (mode=writer): only one dsh process may write these sessions. ` +
            `Stop the running instance, point this one at a different DSH_HOME, or configure mode: readonly for a read-only view.`,
        )
      }
      ...
      lock.degrade()
      logger.warn?.(
        `dsh-collab-sync: writer lock held by pid ${holder.pid ?? '?'} @ ${holder.hostname ?? '?'}` +
          (holder.port ? ` (port ${holder.port})` : '') +
          ' — degrading to READ-ONLY follower (serving UI, refusing writes).\n' +
          '  The follower may promote itself to writer on demand when it receives a message.\n' +
          '  For live multi-terminal sync, prefer ONE backend bound to 0.0.0.0 (see /collab/settings).',
      )
    }
  }
  ...
  if (cfg.repairOnBoot && lock.isWriter) {
    ...
    logger.info?.(`dsh-collab-sync: boot scan — scanned ${s.scanned}, clean ${s.clean}, repaired ${s.repaired}, skipped ${s.skipped}, failed ${s.unrecoverable}`)
  }

  // ── 3) 写路径守卫（appendBatch 拒写 + 读取前修复）─────────────────────────
  installGuard(ctx, {
    assertCanWrite: () => lock.assertCanWrite(),
  ...
  if (cfg.presence) {
    ...
    register({ kind: 'exact', path: '/collab/settings', handler: createSettingsHandler() })
    register({
      kind: 'exact',
    ...
    const tapDisposer = ctx.webServer.tapIndex((html) => {
      if (html.includes('/collab/beacon.js')) return html
      return html.replace('</body>', `${beaconTag()}</body>`)
    })
    disposers.push(tapDisposer)
    ...
    ctx.effect(
      ...
      'dsh-collab-sync: routes + beacon + activity',
    )
  }
      ...
      defineTool({
        ...
        execute: (args = {}) => {
          ...
          if (typeof args.sessionId === 'string' && args.sessionId.length > 0) {
            ...
            if (live) return JSON.stringify({ status: 'skipped', reason: 'session is live; repair at next backend boot' }, null, 2)
