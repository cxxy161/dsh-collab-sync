...
<header>
  ...
  <a href="/collab/panel" target="_blank">协作面板</a>
</header>
<main>
  <section>
  ...
  <section>
    ...
    <div class="row">
      <button id="save">保存并热重载</button>
      <span class="muted">保存会重写 profile 补丁（原文件自动备份），注释可能丢失</span>
    </div>
    <div id="status"></div>
  </section>
</main>
  ...
  function load() {
    fetch('/collab/api/bind').then(function (r) { return r.json() }).then(function (d) {
      ...
      $('schema-note').textContent = d.schemaWidened
        ...
        : 'host schema 未放宽：指定 IP 绑定需要先运行 scripts/setup-bind.sh（或安装时执行过一次 patch-dsh.sh）'
      // 回显当前选择
