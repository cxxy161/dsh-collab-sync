...
export class WriterLock {
  ...
  /**
   * 每次落盘前的守卫：本进程必须仍是锁主人，且锁文件仍是自己的 token。
   * 失败即抛错（宁可大声失败，不再静默损坏日志）。
   */
  assertCanWrite() {
    ...
    if (this.mode === 'readonly') {
      throw new Error(
        'dsh-collab-sync: read-only follower refuses to write the shared session log ' +
          '(mode=readonly); run the single writer backend instead',
      )
    }
    if (!this.owned) {
      throw new Error(
        'dsh-collab-sync: writer lock is not held; refusing to append the session log ' +
          '(another backend may own this session root)',
      )
    }
    if (!this.#ownsToken()) {
      throw new Error(
        'dsh-collab-sync: writer lock was replaced/stolen; refusing to append the session log ' +
