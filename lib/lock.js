...
export class WriterLock {
  ...
  /**
   * 锁冲突时降级为只读跟随者（不持锁、不写）。
   * 调用后 canWrite() 按 readonly 语义拒绝写入。
   */
  degrade() {
    this.mode = 'readonly'
    this.degraded = true
  ...
  /**
   * 写前守卫：本进程必须是写者锁主人。自愈：锁丢失/降级跟随者被使用时按需
   * 重新取锁。**永不抛错、永不阻断回合**：无法写入时返回 {ok:false}，由调用方
   * 响亮告警并跳过本次落盘（内存回合继续，文件保持一致，不损坏）。
   */
  canWrite() {
    if (this.mode === 'off') return { ok: false, reason: 'off', message: 'dsh-collab-sync: 已禁用（mode=off）' }
    if (!this.owned) {
      ...
      if (!result.ok) {
        ...
        if (result.reason === 'readonly') {
          return {
            ok: false,
            reason: 'readonly',
            message: 'dsh-collab-sync: 只读跟随者无法写入共享会话日志（mode=readonly）；请到写者实例上发消息',
          }
        }
        ...
        return {
          ...
          message:
            ...
            ' 持有；本次消息未落盘（内存中继续）。请只运行一个后端（0.0.0.0）',
        }
      }
      this.degraded = false
    ...
    if (!this.#ownsToken()) {
      return {
        ok: false,
        reason: 'stolen',
        message: 'dsh-collab-sync: 写者锁已被替换或丢失；本次消息未落盘（内存中继续），避免序列号分叉损坏',
      }
    }
    return { ok: true }
  }
