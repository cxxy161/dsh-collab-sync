 ...
 * - `appendBatch`：每次实际落盘前调用 `assertCanWrite()`（单写者锁校验），
 *   readonly/锁丢失时拒绝写入，杜绝并发写导致的 seq 分叉损坏；
 ...
 * @param {{assertCanWrite:()=>void, ensure:(id:string)=>object}} hooks
 */
      ...
      const fn = async (...args) => {
        ...
        if (typeof sessionId === 'string') {
          try {
            ...
            ctx.logger?.warn?.(`dsh-collab-sync: pre-read repair check failed for ${sessionId}: ${String(error)}`)
          }
        }
        return orig(...args)
      }
