...
> **重中之重**：① 修复非 127.0.0.1 打开设置页空白（远程设备可用配置）；
> ② 设置页「开放 IP / 协作」卡片直接调整开放范围（多设备共同工作的总开关）。
...
### Fixed
- **静默丢数据（严重）**：落盘守卫在写者锁校验失败时从「告警 + 跳过落盘」改为
  **响亮报错**。此前"跳过"会让消息在界面正常显示但永不写盘，进程重启后整段
  聊天记录消失（用户实际遭遇：昨晚 22:47 之后的对话全部丢失）。现在宁可回合
  可见失败，也绝不静默丢弃持久化。
